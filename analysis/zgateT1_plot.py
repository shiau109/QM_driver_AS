...
for ro, data in dataset.data_vars.items():
    ...
    p, e = curve_fit(FqEqn,[0.035,0.08],[4.4,5.34],p0=init,bounds=(lo_b,up_b))

    fq = FqEqn(flux,*p)
    print("fq sweetspot: ",FqEqn(array([0.08]),*p))
    ax[4].plot(flux,fq)
    ax[4].set_title(f"Ec={round(p[0],3)} GHz, Ej={round(p[1],1)} GHz, d={round(p[2],2)}")
    ax[4].vlines([0.035],min(fq),4.4,colors='black',linestyles="--")
    ax[4].vlines([0.08],min(fq),5.34,colors='orange',linestyles="--")
