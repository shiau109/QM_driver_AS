...
def drag_multi_sine_pulse_waveforms(
    ...
    cos_wave = (
        0.5 * amplitude * (2 * np.pi / end_point * 1e9) * np.cos(t * 2 * np.pi / end_point)+
        0.5 * wx2_amp * (2 * np.pi / end_point * 1e9) * np.cos(t * 2 * np.pi / end_point)+
        0.5 * wx3_amp * (2 * np.pi / end_point * 1e9) * np.cos(t * 2 * np.pi / end_point)+
        0.5 * wx4_amp * (2 * np.pi / end_point * 1e9) * np.cos(t * 2 * np.pi / end_point)
    )  # The derivative of sine function
    z = sin_wave + 1j * 0
    if alpha != 0:
        # The complex DRAG envelope:
        z += 1j * cos_wave * (alpha / (anharmonicity - 2 * np.pi * detuning))
...
def double_multi_sine_pulse_waveforms(
    amplitude, length, anharmonicity, detuning=0.0,alpha=0, **kwargs
):
    """
    Creates Cosine based DRAG waveforms that compensate for the leakage and for the AC stark shift.
