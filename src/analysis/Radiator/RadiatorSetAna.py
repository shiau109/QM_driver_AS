...
from src.exp.mm_radiatorSet import meas_raw_dir
from src.exp.ramsey import T2_fitting
from src.analysis.DRtemp import Kelvin_collector
from src.analysis.state_distribution import train_model, create_img
from src.analysis.exp_decay import qubit_relaxation_statistic
from exp.relaxation_time import T1_hist
...
def hist( data, fig=None)->tuple[plt.Figure, float, float, list]:
    """
    ## Return \n
    Figure, mean_T1, std_T1 and all T1 list. T1 unit in us.\n
    * This function also supports T2 data. 
    """
    if fig == None:
        fig, ax = plt.subplots()
    ax:plt.Axes
    new_data = data/1000 # change ns to us
    mean_t1 = mean(new_data)
    std_t1= std(new_data)
    bin_width = mean_t1 *0.05
    start_value = mean(new_data)*0.5
    end_value = mean(new_data)*1.5
    ...
    plt.title(f"$\tau$ = {round(mean_t1,1)} $\pm$ {round(std_t1,1)} us")
    return fig, mean_t1, std_t1, list(new_data)

def hist_plot(data, rep, time, fit_answer_array:ndarray):
    """
    data shape in (2, N, M):\n
    2 is I and Q , "mixer".\n
    N is repetitions, "repetition".\n
    M is time, "time".\n
    """
    idata = data[0]
    qdata = data[1]
    ...
    ax[0].plot(fit_answer_array, rep)
    ax[1].set_title("Q signal")
    ax[1].pcolormesh(time, rep, qdata, cmap="RdBu")
...
# ============================================ Analysis ================================================================================
def get_references_from_ResultJson(temperature_folder_path:str):
    ...
    for keyname in every_values_dict: # "T1s", "T2s", ...
        values = every_values_dict[keyname]
            ...
            match exp_type:
                case "readout":
                    ...
                    for ro_name, data in dataset.data_vars.items():
                        ...
                        fig, efft, thermalPopu = create_img(new_data, gmm_model, transi_freq, fig)
                        effT_mK.append(efft) # in mK
                        therm_pop.append(thermalPopu*100) # in %
                        plt.savefig(os.path.join(SS_folder,f"{target_q}_OneShot_({exp_idx}).png"))
                        plt.close()
                ...
                case "T1":
                    ...
                    for ro_name, data in dataset.data_vars.items():
                        ...
                        fig = hist_plot(data, rep, time, result["tau"].values)
                        _, _, _, every_T1 = hist(result["tau"].values)
                        T1_us += every_T1
                        gamma1_MHz += list(1/array(every_T1))

                    plt.savefig(os.path.join(T1_pic_folder,f"{target_q}_T1histo_({exp_idx}).png"))
                    plt.close()
                ...
                case "T2":
                    ...
                    for ro_name, data in dataset.data_vars.items():
                        ...
                        for exp_idx in range(chennel.shape[0]):
                            signal = chennel[exp_idx]
                            T2_us.append(round(T2_fitting(time,signal)*1e-3,1))
                            
                        fig, _, _, _ = hist(array(T2_us))
                        
                        gamma2_MHz += list(1/array(T2_us))

                    plt.savefig(os.path.join(T2_pic_folder,f"{target_q}_T2histo_({exp_idx}).png"))
                    plt.close()

                case _:
                    print(f"A recieved exp_type={exp_type} can't be recognized!")
