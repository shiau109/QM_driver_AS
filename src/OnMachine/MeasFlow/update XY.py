...
for i in update_info:
    ...
    update_controlFreq(config_obj, spec.update_aXyInfo_for(target_q=xy_name,IF=ref_IF,LO=qubit_LO))
    ...
    spec.update_aXyInfo_for(target_q=xy_name, amp=pi_amp, len=pi_len)
    update_controlWaveform(spec.get_spec_forConfig("xy"),xy_name=xy_name)
