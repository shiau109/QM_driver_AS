...
def play_sequence(sequence_list:list, q_name:str,number_of_gates:int, pi_len:int):
    i = declare(int)
                ...
                with case_(ii):
                    for iii in range(len(single_qubit_gate_pairs[ii])):
                        if single_qubit_gate_pairs[ii][iii] == "I":
                            ...
                            play(single_qubit_gate_pairs[ii][iii], q_name)


# ###################
# # The QUA program #
# ###################
def RB_executor(pi_len:int,max_circuit_depth:int,delta_clifford:int,q_name:str,ro_elements:list,config:dict,qmm:QuantumMachinesManager,num_of_sequences:int,n_avg:int,ge_threshold:float=0.0,initializer=None,state_discrimination:list=None,simulate:bool=False):
    state_described = False
    if state_discrimination is not None:
        state_described = True
    ...
    with program() as rb:
        ...
        with for_(random_sequence_index, 0, random_sequence_index < num_of_sequences, random_sequence_index + 1):
            ...
            sequence_pairs, sequence_pairs_lengths, recovery_pairs = generate_sequence(max_circuit_depth)
            ...
            with for_(depth, 1, depth <= max_circuit_depth, depth + 1):
                ...
                with if_((depth == 1) | (depth == depth_target)):
                    ...
                    with for_(n, 0, n < n_avg, n + 1):  # Averaging loop
                        ...
                        with strict_timing_():
                            play_sequence(sequence_pairs, q_name, seq_length, pi_len)
                        ...
                        if state_described:
                            assign(state, iqdata_stream[0][0] > ge_threshold)
                            save(state, state_st)
...
if __name__ == '__main__':
    ...
    x, value_avg, error_avg = RB_executor( f"{target_q}_xy",[f"{target_q}_ro"], xyw, max_circuit_depth,300,30, delta_clifford, dyna_config.get_config(), qmm, initializer=init_macro )
