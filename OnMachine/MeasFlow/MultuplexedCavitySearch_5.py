...
if simulate:
    ...
    while results.is_processing():
        ...
        phase = np.zeros((qubit_num, len(dfs)))
        Frequency = np.zeros((qubit_num, len(dfs)))
        ...
        for i in range(qubit_num):
            ...
            R[i] = np.abs(S)
            phase[i] = np.angle(S)
            Frequency[i] = resonator_LO + resonator_IF[i] + dfs
            # Plot R:
            ...
            plt.cla()
            plt.plot((resonator_IF[i] + dfs) / u.MHz, signal.detrend(np.unwrap(phase[i])))
    ...
    if save_data == True:
        ...
        np.savez(save_path, F=Frequency, I=I, Q=Q, R=R, P=signal.detrend(np.unwrap(phase)))
