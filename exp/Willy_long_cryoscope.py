...
def long_cryoscope(flux_Qi,resonator,cryo_const_flux_len,simulate,qmm):
    with program() as cryoscope:
        ...
        with for_(n, 0, n < n_avg, n + 1):
            with for_(*from_array(t, durations)):
                with for_each_(flag, [True, False]):
                    ...
                    align()
                    wait(200 * u.ns)
                    play("const" * amp(cryo_const_flux_amp), f"q{flux_Qi}_z", duration=t)
                    align()
                    wait(200 * u.ns)
                    with if_(flag):
                        play("x90", f"q{flux_Qi}_xy")
        ...
        with stream_processing():
            n_st.save("n")
            for i in resonator_index:
                I_st[i].buffer(2).buffer(len(durations)).average().save(f"I{i}")
                state_st[i].boolean_to_int().buffer(2).buffer(len(durations)).average().save(f"state{i}")                
    ...
    if simulate:
        ...
        while results.is_processing():
            ...
            phase = phase - phase[-1]
            detuning = signal.savgol_filter(phase / 2 / np.pi, 21, 2, deriv=1, delta=0.004) # 1 clock cycle = 4 ns
...
cryo_const_flux_len = 1500
cryo_const_flux_amp = 0.06 / const_flux_amp
...
I, state, phase, detuning, step_response_freq, step_response_volt   = long_cryoscope(flux_Qi,resonator,cryo_const_flux_len,simulate,qmm)
...
ds.to_netcdf("long_cryo.nc", engine='netcdf4', format='NETCDF4')