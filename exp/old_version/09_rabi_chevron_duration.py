...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"Q2 (f_res: {(qubit_LO_q1 + qubit_IF[0]) / u.MHz} MHz)")
        plt.subplot(245)
        plt.cla()
        ...
        plt.title(f"Q3 (f_res: {(qubit_LO_q2 + qubit_IF[1]) / u.MHz} MHz)")
        plt.xlabel("Qubit pulse duration [ns]")
        ...
        plt.title(f"Q4 (f_res: {(qubit_LO_q1 + qubit_IF[2]) / u.MHz} MHz)")
        plt.subplot(247)
        plt.cla()
        ...
        plt.title(f"Q4 (f_res: {(qubit_LO_q2 + qubit_IF[3]) / u.MHz} MHz)")
        plt.subplot(248)
        plt.cla()
